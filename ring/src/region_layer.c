...
void region_layer_draw_boxes(region_layer_t *rl, callback_draw_box callback)
{
    ...
    for (int i = 0; i < rl->boxes_number; ++i)
    {
        int classIndex  = max_index(rl->probs[i], rl->classes);
        float prob = rl->probs[i][classIndex];
        ...
        if (prob > threshold)
        {
            ...
            // drawboxes(x1, y1, x2, y2, classIndex, prob);
            callback(x1, y1, x2, y2, classIndex, prob);
        }
    }
}
